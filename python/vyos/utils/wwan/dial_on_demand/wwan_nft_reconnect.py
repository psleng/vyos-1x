...
class PacketHandler:
    def __init__(self, interface='wwan0', connect_timeout=30, loop=None):
    ...
    async def handle_packet_async(self, packet):
        ...
        if await self.client.wait_for_bearer(int(self.interface[4:]), "connected", timeout=float(5))  == True:
...
async def main(interface='wwan0', timeout=30, connect_timeout=30, loop=None):
    ...
    try:
        handler = PacketHandler(interface, connect_timeout, loop=loop)
        await handler.setup()
        ...
        looper = LoopFunction(loop=loop)
        worker_task = loop.create_task(looper.packet_consumer(queue_num))
