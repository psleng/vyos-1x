...
def main():
    ...
    # secure_grub: CONSUME the prebuilt enforcing core (built in vyos-build by
    # 25-igos-grub-core.chroot) -- copy it onto the ESP. grub-install is SKIPPED
    # above: a self-contained monolithic core needs no on-disk module tree, and the
    # /boot/grub config was already generated by setup_grub/version_add. Same file
    ...
    if get_image_secure_grub():
        ...
        log('secure_grub: installing prebuilt enforcing GRUB core to ESP EFI/VyOS/grubaa64.efi')
